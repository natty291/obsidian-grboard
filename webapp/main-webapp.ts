...
// ══════════════════════════════════════════
//  i18n dictionary (inlined from strings.ts)
// ══════════════════════════════════════════
...
const STRINGS: Record<'ja' | 'en', StringsDict> = {
  ja: {
    cmdName:          '盤面検出ビューを開く',
    ...
    downloadDone:     (f: string) => `✔ ${f} をダウンロード`,
  ...
  en: {
    ...
    downloadDone:     (f: string) => `✔ Downloaded ${f}`,
...
interface AppSettings {
  language: 'ja' | 'en';
  boardBgColor: string;
  markerBlackColor: string;
  markerWhiteColor: string;
...
// ══════════════════════════════════════════
function showNotice(msg: string, durationMs = 3000): void {
...
// ══════════════════════════════════════════
function showConfirm(title: string, message: string, okLabel: string, cancelLabel: string, onConfirm: () => void): void {
...
class BdDetectApp {
  ...
  // ── Correction mode ──
  private correctionMap: Map<string, StoneColor> = new Map();
  ...
  t(key: string, ...args: any[]): string {
    ...
    return typeof val === 'function' ? val(...args) : (val as string ?? key);
  }
  ...
  private el(id: string): HTMLElement {
    return this.rootEl.querySelector('#' + id) as HTMLElement;
  }
  ...
  mount(container: HTMLElement): void {
    ...
    if (sub) sub.textContent = this.t('headerSubtitle');
  }
  ...
  private mk<K extends keyof HTMLElementTagNameMap>(
    parent: HTMLElement,
    tag: K,
    opts: { cls?: string; text?: string; id?: string; style?: string; attr?: Record<string, string> } = {}
  ...
  private buildUI(root: HTMLElement) {
    ...
    const langSel = this.mk(langBar, 'select') as HTMLSelectElement;
    ...
    const optJa = document.createElement('option'); optJa.value = 'ja'; optJa.textContent = '日本語'; langSel.appendChild(optJa);
    const optEn = document.createElement('option'); optEn.value = 'en'; optEn.textContent = 'English'; langSel.appendChild(optEn);
    langSel.value = this.settings.language;
    langSel.addEventListener('change', () => {
      this.settings.language = langSel.value as 'ja' | 'en';
  ...
  private buildStartOverlay(container: HTMLElement, rootEl: HTMLElement) {
    ...
    const fileInput = this.mk(rootEl, 'input') as HTMLInputElement;
    ...
    const genCols = this.mk(r1, 'input') as HTMLInputElement;
    ...
    const genRows = this.mk(r2, 'input') as HTMLInputElement;
    ...
    const genStoneInput = this.mk(r3, 'input') as HTMLInputElement;
    ...
    const updateStoneTotal = () => {
      ...
      if (!genStoneInput.value) genStoneInput.value = String(Math.round(total * 0.4));
    };
    ...
    btnGenGo.addEventListener('click', () => {
      ...
      if (c > 19 || r > 19) { showNotice(this.t('errMaxSize')); return; }
      if (c < 1 || r < 1)   { showNotice(this.t('errMinSize')); return; }
      const total = c * r;
  ...
  private buildMainContent(container: HTMLElement) {
    ...
    this.srcCanvas = this.mk(wrap, 'canvas') as HTMLCanvasElement;
    this.srcCanvas.width = 360; this.srcCanvas.height = 360;
    this.srcCtx = this.srcCanvas.getContext('2d')!;

    this.cropOverlay = this.mk(wrap, 'canvas') as HTMLCanvasElement;
    ...
    const dropInput = this.mk(container, 'input') as HTMLInputElement;
    ...
    (btnDetect as HTMLButtonElement).style.margin = '0';
    btnDetect.addEventListener('click', () => this.onDetect());
    const btnCorrection = this.mk(btnRow, 'button', { cls: 'bd-btn', text: this.t('btnCorrection') }) as HTMLButtonElement;
    ...
    (imgOverlayRow as HTMLElement).style.cssText = 'display:flex;gap:8px;align-items:center;margin-top:6px;flex-wrap:wrap;';
    const btnImgOverlay = this.mk(imgOverlayRow, 'button', { cls: 'bd-btn', text: this.t('btnImageOverlay') }) as HTMLButtonElement;
    (btnImgOverlay as HTMLElement).style.cssText = 'margin:0;flex-shrink:0;';
    ...
    const overlaySlider = this.mk(imgOverlayRow, 'input') as HTMLInputElement;
    ...
    (overlaySlider as HTMLElement).style.cssText = 'flex:1;min-width:80px;';
    overlaySlider.title = this.t('labelOverlayOpacity');
    const overlayValLabel = this.mk(imgOverlayRow, 'span');
    (overlayValLabel as HTMLElement).style.cssText = 'font-size:0.72rem;color:var(--bd-text-faint);min-width:2.5em;text-align:right;';
    ...
    const msSlider = this.mk(msRow, 'input') as HTMLInputElement;
    ...
    const msNum = this.mk(msRow, 'input') as HTMLInputElement;
    ...
    msNum.addEventListener('input', () => {
      ...
      msNum.value = String(n); msSlider.value = String(n);
      this.settings.markerSizeRatio = n;
      this.redrawMarkers();
    });
    ...
    this.croppedCanvas = this.mk(croppedWrap, 'canvas') as HTMLCanvasElement;
    ...
    this.croppedCanvas.addEventListener('click', (e: MouseEvent) => {
      ...
      if (chosen === 'clear') { this.correctionMap.delete(key); }
      else                    { this.correctionMap.set(key, chosen); }
      this.redrawMarkers();
    });;
    ...
    (resultWrap as HTMLElement).style.cssText = 'position:relative;display:inline-block;';
    const rc = this.mk(resultWrap, 'canvas') as HTMLCanvasElement;
  ...
  private buildColorSettings(container: HTMLElement) {
    ...
    const addColorRow = (parent: HTMLElement, label: string, getValue: () => string, setValue: (v: string) => void) => {
      const row = this.mk(parent, 'div', { cls: 'bd-field-row', style: 'margin-top:8px' });
      this.mk(row, 'label', { text: label });
      const cp = this.mk(row, 'input') as HTMLInputElement;
      cp.type = 'color';
    ...
    addColorRow(summary, '盤面背景色 / Board BG',
      () => this.settings.boardBgColor,
      (v) => { this.settings.boardBgColor = v; });
    addColorRow(summary, '黒石マーカー / Black marker',
      () => this.settings.markerBlackColor,
      (v) => { this.settings.markerBlackColor = v; });
    addColorRow(summary, '白石マーカー / White marker',
      () => this.settings.markerWhiteColor,
      (v) => { this.settings.markerWhiteColor = v; });
  }
  ...
  private buildMarginCard(container: HTMLElement) {
    ...
    const diagLabels: [string, string][] = [
    ...
    margins.forEach(([id, label]) => {
      ...
      this.mk(row, 'label', { text: label });
      const sl = this.mk(row, 'input') as HTMLInputElement;
      sl.type = 'range'; sl.id = 'bd-' + id; sl.min = '0'; sl.max = '200'; sl.value = '0'; sl.style.flex = '1';
      const num = this.mk(row, 'input') as HTMLInputElement;
      ...
      num.addEventListener('input', () => {
        ...
        num.value = String(v); sl.value = String(Math.min(v, parseInt(sl.max)));
        this.drawCropOverlay();
      });
    });
  }
  ...
  private buildExpandCard(container: HTMLElement) {
    ...
    errEl.id = 'bd-expandError'; errEl.style.display = 'none';

    const erc = this.mk(container, 'div', { cls: 'bd-card' });
    erc.id = 'bd-expandResultCard'; erc.style.display = 'none';
    ...
    const ec = this.mk(erc, 'canvas') as HTMLCanvasElement;
  ...
  private buildSgfCard(container: HTMLElement) {
    const card = this.mk(container, 'div', { cls: 'bd-card' });
    card.id = 'bd-sgfCard'; card.style.display = 'none';
    ...
    const plSel = this.mk(plRow, 'select') as HTMLSelectElement;
    plSel.id = 'bd-sgfPL';
    const plOpts: [string, string][] = [['', '—'], ['B', this.t('sgfPLBlack')], ['W', this.t('sgfPLWhite')]];
    ...
    const gmSel = this.mk(gmRow, 'select') as HTMLSelectElement;
    gmSel.id = 'bd-sgfGM';
    const gmOpts: [string, string][] = [['1', this.t('sgfGMGo')], ['4', this.t('sgfGMRenju')]];
    ...
    const ruInp = this.mk(ruRow, 'input') as HTMLInputElement;
    ...
    od.id = 'bd-sgfOutput'; od.style.display = 'none';
    ...
    fb.id = 'bd-copyFeedback'; fb.style.display = 'none';

    const ta = this.mk(od, 'textarea') as HTMLTextAreaElement;
    ta.id = 'bd-sgfText'; ta.readOnly = true;
  ...
  private addNumRow(parent: HTMLElement, label: string, id: string, value: number, min: number, max: number, unitText?: string) {
    ...
    this.mk(row, 'label', { text: label });
    const inp = this.mk(row, 'input') as HTMLInputElement;
    inp.type = 'number'; inp.id = id; inp.value = String(value); inp.min = String(min); inp.max = String(max);
    if (unitText) this.mk(row, 'span', { text: unitText, style: 'font-size:0.75rem;color:var(--bd-text-faint)' });
  }
  ...
  private addSliderRow(parent: HTMLElement, label: string, sliderId: string, min: number, max: number, value: number, valId: string, suffix?: string) {
    ...
    vEl.id = valId;
    if (suffix) this.mk(row, 'span', { text: suffix, style: 'font-size:0.68rem;color:var(--bd-text-faint)' });
  ...
  private drawSampleBoard(bsCols: number, bsRows: number, stoneCount?: number) {
    ...
    const s = this.srcCtx;
    s.fillStyle = this.settings.boardBgColor; s.fillRect(0, 0, W, H);
    for (let i = 0; i < H; i += 5) {
      s.strokeStyle = `rgba(0,0,0,${0.03+0.01*Math.sin(i)})`; s.lineWidth=1;
      s.beginPath(); s.moveTo(0,i); s.lineTo(W,i+2); s.stroke();
    }
    ...
    if (bsCols >= 9 && bsRows >= 9) {
      ...
      [sR,Math.floor(bsRows/2),bsRows-1-sR].forEach(r=>{
        [sC,Math.floor(bsCols/2),bsCols-1-sC].forEach(c=>{
          s.beginPath(); s.arc(ix(c),iy(r),2.5,0,Math.PI*2); s.fillStyle='#5a3800'; s.fill();
        });
      });
    }
    ...
    for (let row = 0; row < bsRows; row++) for (let col = 0; col < bsCols; col++) positions.push([row, col]);
    for (let i = positions.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [positions[i], positions[j]] = [positions[j], positions[i]];
    }
    for (let i = 0; i < n; i++) { const [row, col] = positions[i]; place(row, col, i % 2 === 0 ? 'black' : 'white'); }
  ...
  private cropImage(): {outW:number;outH:number;cw:number;ch:number}|null {
    ...
    if (cw<=0||ch<=0) { showNotice(this.t('noticeMarginTooLarge')); return null; }
  ...
  private detectStones(): CellData[][] {
    ...
    const lum=(x:number,y:number):number|null=>{
      if(x<0||x>=OW||y<0||y>=OH) return null;
      const i=(y*OW+x)*4; return 0.299*px[i]+0.587*px[i+1]+0.114*px[i+2];
    };
    const avgL=(cx:number,cy:number,r:number):number=>{
      let s=0,n=0; const ri=Math.ceil(r);
    ...
    for(let row=0;row<bsRows;row++){
      ...
      for(let col=0;col<bsCols;col++){
        ...
        if(l<blackTh){stone='black';}
        else if(l>whiteTh){score=cScore(cx,cy,sR); stone=score>=ringTh?'white':'none';}
        else{stone='none';}
  ...
  private renderResult(data: CellData[][]) {
    ...
    if(rc){ this.drawBoardCanvas(rc,bsCols,bsRows,data.map(r=>r.map(c=>this.getEffectiveStone(c)))); rc.style.display='block'; }
    this.drawImageOverlay();
  ...
  private redrawMarkers() {
    ...
    data.forEach(rowArr=>rowArr.forEach(c=>{
      ...
      if (stone === 'black' || stone === 'white') {
        ...
        this.croppedCtx.beginPath(); this.croppedCtx.moveTo(c.cx-arm,c.cy-arm); this.croppedCtx.lineTo(c.cx+arm,c.cy+arm); this.croppedCtx.stroke();
        this.croppedCtx.beginPath(); this.croppedCtx.moveTo(c.cx+arm,c.cy-arm); this.croppedCtx.lineTo(c.cx-arm,c.cy+arm); this.croppedCtx.stroke();
      }
    }));
  ...
  private getEffectiveStone(cell: CellData): StoneColor {
    return this.correctionMap.get(`${cell.row},${cell.col}`) ?? cell.stone;
  }

  private onToggleCorrectionMode() {
    if (!this.boardData.length) { showNotice(this.t('errNoDetect')); return; }
  ...
  private drawBoardCanvas(canvas: HTMLCanvasElement, cols: number, rows: number, grid: StoneColor[][]) {
    ...
    const ix=(c:number)=>cW/2+c*cW, iy=(r:number)=>cH/2+r*cH;
    ctx.fillStyle=this.settings.boardBgColor; ctx.fillRect(0,0,W,H);
    for(let i=0;i<H;i+=5){
      ctx.strokeStyle=`rgba(0,0,0,${0.03+0.01*Math.sin(i)})`; ctx.lineWidth=1;
      ctx.beginPath(); ctx.moveTo(0,i); ctx.lineTo(W,i+2); ctx.stroke();
    }
    ...
    if(cols>=9&&rows>=9){
      ...
      [sR,Math.floor(rows/2),rows-1-sR].forEach(r=>{
        [sC,Math.floor(cols/2),cols-1-sC].forEach(c=>{
          ctx.beginPath();ctx.arc(ix(c),iy(r),2.5,0,Math.PI*2);ctx.fillStyle='#5a3800';ctx.fill();
        });
      });
    }
  ...
  private onDetect() {
    ...
    if(cols>19||rows>19){ showNotice(this.t('errMaxSize')); return; }
    if(cols<1||rows<1)  { showNotice(this.t('errMinSize')); return; }
  ...
  private onExpand() {
    ...
    const ec=this.el('bd-expandCanvas') as HTMLCanvasElement|null;
    if(ec) this.drawBoardCanvas(ec,oC,oR,board);
  ...
  private onResetConfirm() {
    showConfirm(
      this.t('resetConfirmTitle'), this.t('resetConfirmMsg'),
      this.t('resetConfirmOk'), this.t('resetConfirmCancel'),
      () => this.onReset()
    );
  }
  ...
  private onReset() {
    ...
    const rc=g('bd-resultCanvas'); if(rc) (rc as HTMLElement).style.display='none';
    this.boardData=[]; this.sourceImage=null; this.generatedBoardAnswer=null;
    this.expandedBoard=null; this.lastSgfBody=''; this.lastRenderedData=[];
  ...
  private onMakeSGF() {
    if(!this.expandedBoard){ showNotice(this.t('noticeNoExpand')); return; }
    ...
    for(let r=0;r<rows;r++) for(let c=0;c<cols;c++){
      if(stones[r][c]==='black') bs.push(coord(c,r));
      else if(stones[r][c]==='white') ws.push(coord(c,r));
    }
  ...
  private onCopySGF() {
    const ta=this.el('bd-sgfText') as HTMLTextAreaElement|null;
    if(!ta||!ta.value) return;
    const text=ta.value;
    const showFB=(msg:string)=>{
      const fb=this.el('bd-copyFeedback');
      if(fb){fb.textContent=msg;fb.style.display='inline';setTimeout(()=>{fb.style.display='none';},2000);}
    };
    if(navigator.clipboard?.writeText) {
      ...
      ta.select(); document.execCommand('copy'); showFB(this.t('copyDone'));
    }
  }

  /** Download SGF file (replaces Vault save) */
  private onDownloadSGF() {
    if(!this.lastSgfBody){ showNotice(this.t('errNoSGF')); return; }
    const d=new Date();
  ...
  private loadImageFile(file: File) {
    ...
    reader.onload=(e:ProgressEvent<FileReader>)=>{
      ...
      img.onload=()=>{
        ...
        const rc=this.el('bd-resultCanvas'); if(rc)(rc as HTMLElement).style.display='none';
...
if (typeof window !== 'undefined') {
  (window as any).BdDetectApp = BdDetectApp;
  (window as any).BdDetectDefaultSettings = DEFAULT_SETTINGS;
}
